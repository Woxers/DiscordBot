...
class LuckyPermsDatabase:
    ...
    def test_query(cls):
        try:
            ...
            log_error(f'Exception in test_query occured {e}')
            return 0
    ...
    def get_player_uuid(cls, nickname: str):
        try:
            ...
            log_error(f'Exception in get_player_uuid occured {e}')
            return None
    ...
    def get_player_permissions(cls, nickname: str):
        try:
            ...
            for stroke in result:
                permissions.append({'permission': stroke[2], 'enabled': stroke[3], 'expiry': stroke[6]})
            if (permissions == []):
                return None
            return permissions
        except Exception as e:
            log_error(f'Exception in get_player occured {e}')
            return None
